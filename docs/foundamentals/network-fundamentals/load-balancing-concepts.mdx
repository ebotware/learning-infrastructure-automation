...
| Alta disponibilità      | Se un server muore, il traffico viene reindirizzato automaticamente         | Riduzione downtime                         |
| Miglioramento performance | Riduzione della latenza media e del tempo di risposta                      | Migliore UX                                |
...
| Livello OSI | Nome comune        | Cosa osserva                             | Velocità                  | Casi d’uso tipici                              | Esempi moderni                     |
|-------------|-------------------|------------------------------------------|---------------------------|------------------------------------------------|-------------------------------------|
...
**Nota** (da verificare): a quanto pare la maggior parte delle nuove architetture cloud-native utilizza **Layer 7** per le API e **service mesh** (Istio, Linkerd) che includono load balancing avanzato.

## Algoritmi di Load Balancing più comuni

| Algoritmo                  | Tipo     | Come funziona                                      | Quando usarlo                                      | Limiti / Svantaggi                              |
|----------------------------|----------|----------------------------------------------------|----------------------------------------------------|-------------------------------------------------|
| **Round Robin**            | Statico  | Cicla tra i server in ordine fisso                 | Server tutti uguali, carico costante               | Non considera il carico reale                  |
...
| **Least Connections**      | Dinamico | Invia al server con meno connessioni attive       | Richieste di durata variabile (molto usato)       | Non considera il tempo di risposta             |
| **Weighted Least Connections** | Dinamico | Least Connections + pesi                        | Server eterogenei + durata variabile              | Calcolo più costoso                             |
| **IP Hash**                | Statico  | Hash dell’IP client → stesso server sempre       | Session affinity / sticky sessions senza cookie   | Squilibri se molti client da NAT / proxy       |
